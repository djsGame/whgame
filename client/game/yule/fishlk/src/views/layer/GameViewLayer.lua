...
--获取炮台
  ...
  if fishtype >= g_var(cmd).FishType.FishType_JianYu and fishtype <= g_var(cmd).FishType.FishType_LiKui then
    ...
    if goldTxt == nil then
      ...
      goldTxt = cc.LabelAtlas:create(string.format("%d", score),"game_res/mutipleNum.png",24,30,string.byte("0"))
      goldTxt:setAnchorPoint(0.5,0.5)

      goldTxt:setPosition(pos.x, pos.y)          
      self:addChild(goldTxt,6)

      local action = cc.Sequence:create(cc.RotateTo:create(time*6,angle),cc.RotateTo:create(time*6,-angle),cc.RotateTo:create(time*6,0))
      local call = cc.CallFunc:create(function()  
