...
function Fish:schedulerUpdate()
	...
	local function updateFish(dt)
		...
		if self.m_data.nBezierCount <= self.m_pathIndex-1 then 
			if self.m_data.wHitChair == self._scene.m_nChairID  and self.m_data.nFishType == g_var(cmd).FishType.FishType_BaoXiang then
		...
		if cc.rectContainsPoint( cc.rect(0,0,yl.WIDTH, yl.HEIGHT), point ) then
			if self.m_data.nFishType ~= g_var(cmd).FishType.FishType_BaoXiang then
