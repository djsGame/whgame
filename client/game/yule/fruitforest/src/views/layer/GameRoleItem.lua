...
function GameRoleItem:ctor(userItem, viewId)
    ...
    -- 用户头像
    local head = PopupInfoHead:createClipHead(userItem, 115)
   -- head:enableInfoPop(true, popPosition[viewId], popAnchor[viewId])
	...
	self.spHead_bk = cc.Sprite:create("roleItem/headback.png"):addTo(head)
	:setPosition(0, 0)
	--用户昵称
...
--[[   --金币
	...
	self.m_clipScore = ClipText:createClipText(cc.size(150, 20), self.m_userItem.lScore .. "")
    self.m_clipScore:setAnchorPoint(cc.p(0.5, 0.5))
    self.m_clipScore:setPosition(bk_size.width/2 + 10, bk_size.height/2)
...
function GameRoleItem:switchGameState(Coins )
    ...
    --if self.m_nViewId ~= cmd.MY_VIEWID then
        ...
        if nil == self.m_clipScore then
            ...
            self.m_clipScore:setString(Coins.. "")
        end
    --end
