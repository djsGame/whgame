...
function ClientScene:onCreate()
    ...
    --玩家昵称
	--self._name = areaTop:getChildByName("Text_Name")
    self._name = areaTop:getChildByName("text_name")
	self._name:setString(GlobalUserItem.szNickName.."")
	...
	--游戏豆Sprite_9_0 sp_num_bg_2
	areaTop:getChildByName("sp_num_bg_2"):setVisible(false)
	areaTop:getChildByName("Sprite_9_0"):setVisible(false)
	self._bean = areaTop:getChildByName("atlas_bean"):setVisible(false)
	btn = areaTop:getChildByName("btn_charge"):setVisible(false)
	...
	--元宝Sprite_9_0_0 sp_num_bg_2_0
	areaTop:getChildByName("sp_num_bg_2_0"):setVisible(false)
	areaTop:getChildByName("Sprite_9_0_0"):setVisible(false)
	self._ingot = areaTop:getChildByName("atlas_ingot"):setVisible(false)
	btn = areaTop:getChildByName("btn_exchange"):setVisible(false)
    btn:setTag(ClientScene.BT_RECHARGE)
    btn:addTouchEventListener(btcallback)

    --宝箱
    self.m_btnBox = areaTop:getChildByName("btn_box"):setVisible(false)
    self.m_btnBox:setTag(ClientScene.BT_BOX)
    self.m_btnBox:addTouchEventListener(btcallback)
	local box = ExternalFun.loadCSB("plaza/BoxAni.csb", areaTop):setVisible(false)
	...
	--任务	
	btn = areaBottom:getChildByName("btn_task"):setVisible(false)
	...
	btn = self._notify:getChildByName("btn_trumpet"):setVisible(false)
...
function ClientScene:onEnterRoom()
	...
	if GlobalUserItem.isAntiCheat() then
		...
		if self._gameFrame:SitDown(yl.INVALID_TABLE,yl.INVALID_CHAIR) then
			self:showPopWait2("正为您配桌, 请稍后...", function()
				self._gameFrame:onCloseSocket()
			end)
		end
		return
	...
	if not tag then
		...
		if cur == yl.SCENE_GAME then
			...
			elseif GlobalUserItem.bPrivateRoom then
				...
				self._gameFrame:onCloseSocket()		
			-- 录像
			elseif GlobalUserItem.bVideo then
				print("录像处理")
				-- 清理记录
				self._sceneRecord = {}
				self._sceneRecord[1] = yl.SCENE_GAMELIST
...
function ClientScene:getTagLayer(tag, param)
	...
	elseif tag == yl.SCENE_GAME then
		...
		if nil ~= entergame then
			...
			if GameVideo and nil ~= dst and true == GlobalUserItem.bVideo then
				GameVideo:getInstance():enterGame(dst, self, param)
			end
		else
			print("游戏记录错误")
		end			
	...
	elseif GameVideo and tag > GameVideo:getInstance().LAYTAG.LAYER_BEGIN and tag < GameVideo:getInstance().LAYTAG.LAYER_END then
		dst = GameVideo:getInstance():getTagLayer(tag, param, self)
	end
	if dst then
		dst:setTag(tag)
	end
