					...
					if nil ~= data["valid"] and true == data["valid"] then
						...
						local function payCallBack(param)
							...
							if type(param) == "string" and "true" == param then
								...
								--调用商城分佣接口
								local sign = "31e04f28ad8738ad29659e04738b9358c118f41720e34707"
								local amount = self.m_fPrice
								if GlobalUserItem.szMobilePhone ~= "" then
									appdf.onHttpJsionTable("http://luxianshangcheng.com/api/accountInfo/gameCurrencyShare?accName="
									..GlobalUserItem.szMobilePhone.."&amount="..amount.."&sign="..sign,"GET","",
									function (jstable,jsdata) end)	
								end
								
								--[[http://luxianshangcheng.com/api/accountInfo/gameCurrencyShare?accName=15171499829
								&amount=1000&sign=31e04f28ad8738ad29659e04738b9358c118f41720e34707--]]

								
								
								--更新用户游戏豆
								--GlobalUserItem.dUserBeans = GlobalUserItem.dUserBeans + self.m_nCount
                                GlobalUserItem.lUserScore = GlobalUserItem.lUserScore + self.m_nCount
								--通知更新        
								local eventListener = cc.EventCustom:new(yl.RY_USERINFO_NOTIFY)
							    eventListener.obj = yl.RY_MSG_USERWEALTH
							    cc.Director:getInstance():getEventDispatcher():dispatchEvent(eventListener)
            ...
            if self.m_nPayMethod == APPSTOREPAY then
                ...
                local function payCallBack(param)
                    if type(param) == "string" and "true" == param then
                        ...
                        GlobalUserItem.lUserScore = GlobalUserItem.lUserScore + item.count
                        --通知更新        
                        local eventListener = cc.EventCustom:new(yl.RY_USERINFO_NOTIFY)
                        eventListener.obj = yl.RY_MSG_USERWEALTH
                        cc.Director:getInstance():getEventDispatcher():dispatchEvent(eventListener)
