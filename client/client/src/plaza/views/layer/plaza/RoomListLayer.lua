...
--子视图数目
function RoomListLayer:numberOfCellsInTableView(viseew)
...
function RoomListLayer:tableCellTouched(view, cell)
	...
	if view:getParent()._scene:roomEnterCheck()  then
		if roominfo.wKindID == 504 then --森林舞会单独处理
		 	self:getParent():getParent():getApp():enterSceneEx(appdf.CLIENT_SRC .. "plaza.views.ForestDanceScene","FADE",0.2)
		else
...
function RoomListLayer:tableCellAtIndex(view, idx)
	...
	if 8 == wLv then
		...
		if iteminfo.wKindID == 518 or iteminfo.wKindID == 517 then
			wRoom = iteminfo.wSortID - 1
		end
		...
		if iteminfo.wKindID ~= 518 and iteminfo.wKindID ~= 517 then
			--背景
