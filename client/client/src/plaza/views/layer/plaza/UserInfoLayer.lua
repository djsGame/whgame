...
function QrCodeLayer:ctor(scene)
    self._scene = scene
	--注册触摸事件
	...
	local temp = self.m_spBg:getChildByName("qr_sp_title_2"):setVisible(false)
	local tempX,tempY = temp:getPosition()
	local download = cc.Sprite:create("userinfo/download.png")
	download:addTo(self.m_spBg)
	download:setPosition(cc.p(tempX,tempY))
	...
	local qr = cc.Sprite:create("userinfo/kuangbaoewm.png")
	qr:setScale(1.5)
    self.m_spBg:addChild(qr)
    qr:setPosition(cc.p(bgsize.width * 0.5, bgsize.height * 0.5))
	self.m_qrNode = qr
...
function PromoterInputLayer:onButtonClickedEvent( tag, sender )
	...
	elseif BT_QR_ENTER == tag then
        if true == self.m_bOpenKeyboard then
            self.m_bOpenKeyboard = false
            return
        end
		local qrlayer = QrCodeLayer:create(self.m_parent._scene)
...
function UserInfoLayer:ctor(scene)
    ...
    -- 视频回放
    --self.m_btnGameVideo = ccui.Button:create("Information/uinfo_btn_gamevideo_0.png","Information/uinfo_btn_gamevideo_1.png","Information/uinfo_btn_gamevideo_0.png")
    --self.m_btnGameVideo:move(yl.WIDTH-90,yl.HEIGHT-51)
    --self.m_btnGameVideo:setTag(UserInfoLayer.BT_GAMEVIDEO)
    --self.m_btnGameVideo:addTo(self)
    --self.m_btnGameVideo:addTouchEventListener(btcallback)
    --self.m_btnGameVideo:setVisible(false)
    --self.m_btnGameVideo:setEnabled(false)
	...
	cc.Label:createWithTTF("+ "..GlobalUserItem.lUpgradeRewardIngot.." 元宝","fonts/round_body.ttf", 18)
		:move(165+20,120+10)
		:setAnchorPoint(cc.p(0,0.5))
		:setTextColor(cc.c3b(250,250,0)):setVisible(false)
		:addTo(self)
	...
	display.newSprite("Information/text_info_gold.png")
		:move(518,232)
		:addTo(self)
	display.newSprite("Information/text_info_bean.png"):setVisible(false)
		:move(518,155)
		:addTo(self)
	display.newSprite("Information/text_info_ingot.png"):setVisible(false)
		:move(533,80)
		:addTo(self)
	...
	self._txtBean:setPosition(590, 155):setVisible(false)
	...
	self._txtIngot:setPosition(590, 80):setVisible(false)
    ...
    if 0 == vip then
    	--开通VIP
		ccui.Button:create("Information/bt_info_vip_0.png","Information/bt_info_vip_1.png"):setVisible(false)
	    	:move(1064,537)
    ...
    ccui.Button:create("Information/bt_info_recharge_0.png","Information/bt_info_recharge_1.png"):setVisible(false)
    	:move(1045,155)
    	:setTag(UserInfoLayer.BT_RECHARGE)
    	:addTo(self)
    	:addTouchEventListener(btcallback)

    ccui.Button:create("Information/bt_info_exchange_0.png","Information/bt_info_exchange_1.png"):setVisible(false)
    	:move(1045,80)
...
function UserInfoLayer:onButtonClickedEvent(tag,ref)
	...
	elseif tag == UserInfoLayer.BT_PROMOTER then
		--local prolayer = PromoterInputLayer:create(self)
		--self:addChild(prolayer)
    elseif tag == UserInfoLayer.BT_GAMEVIDEO then
        if GameVideo then
            GameVideo:getInstance():downloadRecordFile(111)
        end
	end
end
