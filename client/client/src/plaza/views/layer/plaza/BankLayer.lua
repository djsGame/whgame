...
local PopInfoDialog = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.PopInfoDialog")
...
BankLayer.BT_GOLD 			= 21
BankLayer.BT_PLATINUM  		= 22
BankLayer.BT_DIAMOND 		= 23
...
function BankLayer:ctor(scene, gameFrame)
	...
	self.m_tabBankConfigInfo.wRevenueTake = 0
	self.m_tabBankConfigInfo.wRevenueTransfer = 0
	...
	self._cbtTransfer:setVisible(true)
	self._cbtTransfer:setEnabled(true)
	self._cbtTransfer:setEnabled(true)
	self._cbtTransfer:setVisible(true)
      self._cbtSaveTake:setEnabled(true)
       self._cbtSaveTake:setVisible(true)
    ...
    ccui.Button:create("Bank/bt_bank_check.png","")
    	...
    	:addTo(self._takesaveArea)
    	:addTouchEventListener(btcallback)
    self._notifyText = cc.Label:createWithTTF("存款无需输入银行密码。", "fonts/round_body.ttf", 24)
								:addTo(self._takesaveArea)
	...
	self.m_selected = 0	--选择黄金为1，白金为10，钻石为50
	--黄金皇冠	
	local gold = ccui.CheckBox:create("Bank/gold0.png","","Bank/gold1.png","","")
	:setTag(BankLayer.BT_GOLD)
	gold:addTo(self._transferArea)
	gold:setPosition(cc.p(1334/3,335+90+10))
	 :addTouchEventListener(btcallback)
	local goldTTF = cc.Label:createWithTTF("黄金皇冠","fonts/round_body.ttf",24)
	goldTTF:addTo(gold)
	goldTTF:setPosition(cc.p(85/2,135))
	local silverTTF = cc.Label:createWithTTF("18万5","fonts/round_body.ttf",24)
	silverTTF:addTo(gold)
	silverTTF:setPosition(cc.p(85/2,42))
	--白金皇冠
	local platinum = ccui.CheckBox:create("Bank/platinum0.png","","Bank/platinum1.png","","")
	:setTag(BankLayer.BT_PLATINUM)
	platinum:addTo(self._transferArea)
	platinum:setPosition(cc.p(1334/3+100+80-3,335+90+10))
	 :addTouchEventListener(btcallback)
	local goldTTF = cc.Label:createWithTTF("白金皇冠","fonts/round_body.ttf",24)
	goldTTF:addTo(platinum)
	goldTTF:setPosition(cc.p(85/2,135))
	local silverTTF = cc.Label:createWithTTF("185万","fonts/round_body.ttf",24)
	silverTTF:addTo(platinum)
	silverTTF:setPosition(cc.p(85/2,42))
	--钻石皇冠
	local diamond = ccui.CheckBox:create("Bank/diamond0.png","","Bank/diamond1.png","","")
	:setTag(BankLayer.BT_DIAMOND)
	diamond:addTo(self._transferArea)
	diamond:setPosition(cc.p(1334/3+200+160-6,335+90+10))
	 :addTouchEventListener(btcallback)
	local goldTTF = cc.Label:createWithTTF("钻石皇冠","fonts/round_body.ttf",24)
	goldTTF:addTo(diamond)
	goldTTF:setPosition(cc.p(85/2,135))
	local silverTTF = cc.Label:createWithTTF("200万","fonts/round_body.ttf",24)
	silverTTF:addTo(diamond)
	silverTTF:setPosition(cc.p(85/2,42))

    display.newSprite("Bank/text_bank_preAccount.png")
    	:move(300,425-90-25)
    	:addTo(self._transferArea)
    display.newSprite("Bank/text_bank_preNumber.png")
    	:move(300,335-90-50)
    	:addTo(self._transferArea)
--[[		
	--银行密码
...
--[[
    ...
    ccui.Button:create("Bank/bt_bank_check.png","")
    	...
    	:addTo(self._transferArea)
    	:addTouchEventListener(btcallback)
    self._notifyTextPresent = cc.Label:createWithTTF("存款无需输入银行密码。", "fonts/round_body.ttf", 24)
								:addTo(self._transferArea)
								:setTextColor(cc.c4b(136,164,224,255))
								:move(1250/2,38)
								:setVisible(false)
...
function BankLayer:onButtonClickedEvent(tag,sender)
	...
	elseif tag == BankLayer.BT_GOLD then
		--self._transferArea:getChildByTag(BankLayer.BT_GOLD):setSelected(false)
		self._transferArea:getChildByTag(BankLayer.BT_PLATINUM):setSelected(false)
		self._transferArea:getChildByTag(BankLayer.BT_DIAMOND):setSelected(false)
		self.m_selected = 185000
	elseif tag == BankLayer.BT_PLATINUM then
		self._transferArea:getChildByTag(BankLayer.BT_GOLD):setSelected(false)
		--self._transferArea:getChildByTag(BankLayer.BT_PLATINUM):setSelected(false)
		self._transferArea:getChildByTag(BankLayer.BT_DIAMOND):setSelected(false)
		self.m_selected = 1850000
	elseif tag == BankLayer.BT_DIAMOND then
		self._transferArea:getChildByTag(BankLayer.BT_GOLD):setSelected(false)
		self._transferArea:getChildByTag(BankLayer.BT_PLATINUM):setSelected(false)
		--self._transferArea:getChildByTag(BankLayer.BT_DIAMOND):setSelected(false)
		self.m_selected = 2000000
	end
end

function BankLayer:onGold()
...
function BankLayer:onSelectedEvent(sender,eventType)
	...
	if tag == BankLayer.CBT_SAVEORTAKE or tag == BankLayer.CBT_TRANSFER then
		...
		if transfermode then 
            ...
            self._notifyText:setString(str):setVisible(false)
		end        
...
function BankLayer:showDialog(_lOperateScore,_szTarget,_szPass,_byID)
	print("showDialog")
	print("---------self.m_selected = ",self.m_selected)
	local str = ""
    local _lType = 0
	if tonumber(self.m_selected)  == 185000 then
		str = "黄金皇冠"
	elseif  tonumber(self.m_selected) == 1850000 then	
		str = "白金皇冠"
        _lType = 1
    elseif tonumber(self.m_selected) == 2000000 then
		str = "钻石皇冠"
        _lType = 2
	end

	PopInfoDialog:create("系统提示","赠送ID:","礼物类型:","赠送数量:",_szTarget,str,self._szScore,function(bReTry)
			if bReTry == true then
				self:showPopWait()
				self._bankFrame:onTransferScore(_lOperateScore,_szTarget,_szPass,_byID,_lType,self._szScore)
			else
--				os.exit(0)
			end
		end)
...
function BankLayer:onTransferScore()
	...
	local lOperateScore = tonumber(szScore * self.m_selected)
	self._szScore = szScore
	if lOperateScore<1 then
		showToast(self,"请输入正确金额！",2)
		return
	end
...
--[[	self:showPopWait()
	...
	self:showDialog(lOperateScore,szTarget,szPass,byID)
end
