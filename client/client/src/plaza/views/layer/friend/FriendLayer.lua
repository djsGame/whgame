...
function FriendLayer:initWithButton()
    ...
    if not yl.LOCATION_SUPPORT then
    ...
    btn:addTouchEventListener(function(ref, tType)
        if tType == ccui.TouchEventType.ended then 
            ...
            if targetPlatform == cc.PLATFORM_OS_ANDROID then
                msg = "亲爱的好友，我最近玩了一款超好玩的游戏，玩法超级多，内容超级精彩，快来加入我，和我一起精彩游戏吧！下载地址："
            end 
            MultiPlatform:getInstance():shareToTarget(yl.ThirdParty.SMS, sharecall, "好友分享", msg, url)
        end
    end)
    if not yl.LOCATION_SUPPORT then
        btn:setPosition(nearbtn:getPosition())
    end 
...
function FriendLayer:searchResult(userTab)
    ...
    if #userTab ==0 then
        ...
        for i=1,#userTab do   
            ...
            local lValue = cc.LabelAtlas:_create(tostring(userInfoTab.wGrowLevel),"Friend/shuzi1.png",16,20,string.byte("0"));
