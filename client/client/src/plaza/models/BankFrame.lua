...
function BankFrame:onConnectCompeleted()
	...
	elseif self._oprateCode == BankFrame.OP_CAOZUO then			--发送操作
		self:sendCaoZuoData()
	else
		self:onCloseSocket()
...
function BankFrame:onSocketEvent(main,sub,pData)
	...
	if main == logincmd.MDM_GP_USER_SERVICE then --用户服务
		...
		elseif sub == game_cmd.SUB_GP_USER_CHAOZUOJIEGUO then 	--接受赠送操作结果
			local cmd_table = ExternalFun.read_netdata(game_cmd.CMD_GP_UserChaozuoJieguo, pData)
			self._callBack(cmd_table.cbJiegguo,"")
		elseif sub == logincmd.SUB_GP_OPERATE_FAILURE_USER then
			self._callBack(-1,"操作失败")
		else
...
function BankFrame:onGameSocketEvent(main,sub,pData)
	if main == game_cmd.MDM_GR_INSURE then
		...
		elseif sub == game_cmd.SUB_GP_USER_CHAOZUOJIEGUO then 	--开通结果
			local cmd_table = ExternalFun.read_netdata(game_cmd.CMD_GP_UserChaozuoJieguo, pData)
			dump(cmd_table, "SUB_GP_USER_CHAOZUOJIEGUO")
--[[			local eventListener = cc.EventCustom:new(yl.RY_USERINFO_NOTIFY)
		    eventListener.obj = yl.RY_MSG_USERWEALTH
		    cc.Director:getInstance():getEventDispatcher():dispatchEvent(eventListener)--]]
		end
...
function BankFrame:sendCaoZuo(_state,_record)
	self._oprateCode = BankFrame.OP_CAOZUO
	self._state = _state
...
function BankFrame:sendCaoZuoData()
	local TakeData = CCmd_Data:create(9)
	TakeData:setcmdinfo(yl.MDM_GP_USER_SERVICE,yl.SUB_GP_USER_CHAOZUO)
	TakeData:pushdword(self._record)
	TakeData:pushdword(GlobalUserItem.dwUserID)
	TakeData:pushbyte(self._state)
		--发送失败
...
function BankFrame:sendTransferScore()
	local TransferScore = CCmd_Data:create(280)
    ...
    TransferScore:pushint(self._lType)
    TransferScore:pushint(self._szScore)
...
function BankFrame:onTransferScore(lScore,target,szPassword,byID,lType,szScore)
	self._tabTarget = {}
