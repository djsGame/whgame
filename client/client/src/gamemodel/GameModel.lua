...
function GameModel:addVideoControlLayer( layer )
    ...
    if nil ~= self._gameView.addToRootLayer then
        self._gameView:addToRootLayer(layer, self:videoLayerZorder())
    else
        self._gameView:addChild(layer, self:videoLayerZorder())
    end
end

-- 回放层zorder
function GameModel:videoLayerZorder()
    if nil ~= self._gameView and nil ~= self._gameView.videoLayerZorder then
        return self._gameView:videoLayerZorder()
    end
    return yl.MAX_INT
end
