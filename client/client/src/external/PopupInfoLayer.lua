...
function PopupInfoLayer:ctor(viewParent, isGame)
	...
	self.m_bEnableLocationInfo = false

	--加载csb资源
	isGame = isGame or false
	local csbNode = nil
	if true == isGame then
		if yl.LOCATION_SUPPORT and yl.SHOW_IP_ADDRESS then
			self.m_bEnableLocationInfo = true
	...
	--元宝
	self.m_textIngot = bg_kuang:getChildByName("ingot_text"):setVisible(false)

	--游戏币
	self.m_textScore = bg_kuang:getChildByName("score_text")

	--游戏豆
	self.m_textBean = bg_kuang:getChildByName("bean_text"):setVisible(false)

	-- ip地址
	self.m_textIp = bg_kuang:getChildByName("ip_text")
	self.m_textIp:setVisible(self.m_bEnableLocationInfo)

	-- 距离
	self.m_textDistance = bg_kuang:getChildByName("distance_text")	
	self.m_textDistance:setVisible(self.m_bEnableLocationInfo)
	self.m_clipDistance = ClipText:createClipText(cc.size(120, 30),"")
	self.m_clipDistance:setVisible(self.m_bEnableLocationInfo)
	if true == isGame then
...
function PopupInfoLayer:refresh( useritem, popPos ,anr)
	...
	if useritem.dwGameID == GlobalUserItem.dwGameID then
		...
		if not bAntiCheat and nil == dwDistance and yl.LOCATION_SUPPORT then
			-- 请求玩家信息
	...
	self.m_textIp:setVisible( self.m_bEnableLocationInfo and not bAntiCheat )
	...
	self.m_clipDistance:setVisible( self.m_bEnableLocationInfo and not bAntiCheat)
	self.m_textDistance:setVisible( self.m_bEnableLocationInfo and not bAntiCheat)
