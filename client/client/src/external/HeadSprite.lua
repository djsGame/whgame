...
function HeadSprite:updateHead( useritem )
	...
	if true == bAntiCheat then
		...
		if nil ~= render then
			...
			if nil ~= frame then
				local framesize = frame:getRect()
				if framesize.height > 0 and framesize.width > 0 then
...
function HeadSprite:updateHeadByFrame(frame)
	...
	local framesize = frame:getRect()
	if framesize.height < 0 or framesize.width < 0 or framesize.height > 96 or framesize.width > 96 then
		local logtable = {}
		logtable.szMsg = "HeadSprite 头像更新异常"
		logtable.headrect = framesize
		if nil ~= self.m_useritem then
			logtable.dwGameID = self.m_useritem.dwGameID or "null gameid"
			logtable.szNick = self.m_useritem.szNickName or "null nick"
			logtable.wFaceId = self.m_useritem.wFaceID or "null faceid"
			logtable.dwCustomID = self.m_useritem.dwCustomID or "null customid"
			logtable.szThirdUrl = self.m_useritem.szThirdPartyUrl or "null szThirdPartyUrl"
		else
			logtable.szMsg2 = "self.m_useritem 异常"
		end
		...
		if nil ~= jsonStr then
			if cc.PLATFORM_OS_WINDOWS == targetPlatform then
				...
				buglyReportLuaException(jsonStr, debug.traceback())
			end	
		end
		print("头像异常")
		frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(string.format("Avatar%d.png", 0))
	end	
...
function HeadSprite:registerTouch( )
	...
	local function onTouchEnded( touch, event )
        ...
        if true == cc.rectContainsPoint(rec, pos) then
            if type(self.m_fun) == "function" then
            	self.m_fun()
            end
        end        
	end
...
-- 获取头像
function HeadSprite:getHeadSprite( useritem )
	local isThirdParty = useritem.bThirdPartyLogin or false
	--系统头像
	local faceid = useritem.wFaceID or 0
