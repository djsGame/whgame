...
function RoomCardShopLayer:initExchangeUI(csbNode)
    ...
    local szcount = "" .. GlobalUserItem.lRoomCard
    if string.len(szcount) > 6 then
        self.m_textLeft:setString("剩余" .. string.sub(GlobalUserItem.lRoomCard, 1, 6) .. "...")
    else
...
function RoomCardShopLayer:initBuyUI(csbNode)
    ...
    if nil ~= self.m_tabParam.item.ingot and 0 ~= self.m_tabParam.item.ingot then
        local bSelect = false
        if not bHaveBean and not bHaveGold then
